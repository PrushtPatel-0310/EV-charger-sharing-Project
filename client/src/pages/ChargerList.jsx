...
const ChargerList = () => {
  ...
  const [filters] = useState({ page: 1, limit: 200 });
  ...
  const fetchChargers = async () => {
    try {
      setLoading(true);
      const response = await chargerService.getAll({ ...filters, limit: 200 });
  ...
  const chargersOnRoute = useMemo(() => {
    ...
    const line = turf.lineString(routeCoords.map((p) => [p.lng, p.lat]));
    return chargers.filter((charger) => {
      const { lat, lng } = charger.location.coordinates || {};
      if (typeof lat !== 'number' || typeof lng !== 'number') return false;
      const distance = turf.pointToLineDistance(turf.point([lng, lat]), line, { units: 'kilometers' });
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="space-y-4">
        <div className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Plan a route</h2>
        ...
        <div className="grid gap-4 lg:grid-cols-[420px_1fr]">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
              <div className="max-h-[calc(100vh-280px)] overflow-y-auto divide-y divide-gray-100">
                {chargersOnRoute.length === 0 ? (
                  <div className="px-4 py-8 text-center text-gray-500">No chargers found in this area.</div>
                ) : (
                    ...
                    return (
                      ...
                      >
                        {charger.images?.length ? (
                          ...
                          <div className="flex h-20 w-24 items-center justify-center rounded-xl bg-gray-100 text-sm text-gray-500">
                            No image
                          </div>
                        )}
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="text-right">
                              ...
                              {charger.rating > 0 && <p className="text-xs text-yellow-600">⭐ {charger.rating}</p>}
                            </div>
                          </div>
                          <div className="mt-2 flex gap-2 text-xs text-primary-600">
                            <Link to={`/chargers/${charger._id}`} className="font-semibold hover:underline">
                              Details
                            </Link>
                            <Link to={`/chargers/${charger._id}#slots`} className="font-semibold hover:underline">
                              Slots
                            </Link>
                          </div>
                        </div>
          ...
          <div className="sticky top-4 h-[calc(100vh-200px)] overflow-hidden rounded-3xl border border-gray-200 shadow-lg">
            <Map
